...
export default class ClaimForm extends React.Component {
  ...
  fetchInfo = async () => {
    if (this.props.address) {
      let data = await getUnclaimed(this.props.poolId, this.props.signer);
      this.setState({
        claimable: data,
      });
    }
  ...
  render() {
    ...
    return (
      <form className="mt-3 pb-1 px-2 pr-1 rounded-full flex flex-col justify-center items-center w-1/2 flex-1">
        <input
          className="pl-2 md:pl-4 md:pr-2 w-full h-[45px] focus:outline-none text-white border border-[#5F5C65] bg-primary-black rounded-[5px] font-Poppins text-sm"
          placeholder={`Claim: 0.00 $$$$`}
          disabled
          value={claimable}
          type="text"
        />
